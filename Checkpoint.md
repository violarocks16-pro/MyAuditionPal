...
1. **"Patterns to work on" (BUILT, no AI, 2026-06-16)** — when Attended, the form has an **"Areas to work on"** chip multi-select (`constants/improvement-areas.ts` → `improvementAreas: string[]`, cloud col `improvement_areas text[]`). The **History tab** shows a "Patterns to work on" card counting tagged areas across attended auditions, most-frequent first. **Optional future AI report** could build on this structured data via a backend (Supabase Edge Function): paid Claude (Haiku, pennies) or free tiers (Gemini/Groq/Cloudflare). `improvement_report` column reserved.
