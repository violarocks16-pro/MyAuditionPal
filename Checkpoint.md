...
- ✅ **Card animation (DONE 2026-06-16)** — audition + listing cards have a staggered `FadeInDown` entrance (react-native-reanimated, ~420ms + 60ms/card stagger) via an `index` prop. Could add a press/scale animation later.
1. **"Patterns to work on" (BUILT, no AI, 2026-06-16)** — when Attended, the form has an **"Areas to work on"** chip multi-select (`constants/improvement-areas.ts` → `improvementAreas: string[]`, cloud col `improvement_areas text[]`). The **History tab** shows a "Patterns to work on" card counting tagged areas across attended auditions, most-frequent first. **⭐ USER-REQUESTED REMINDER (2026-06-16):** build a **summary report on the History tab that lists what to work on** for auditions (a written summary on top of the current "patterns to work on" counts). Likely the AI prose report — could build on this structured data via a backend (Supabase Edge Function): paid Claude (Haiku, pennies) or free tiers (Gemini/Groq/Cloudflare). `improvement_report` column reserved.
